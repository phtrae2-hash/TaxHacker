...
export function NewsletterForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setStatus("success")
      setMessage("Obrigado por se inscrever! Verifique seu e-mail para confirmar.")
      setEmail("")
    } catch (error) {
      setStatus("error")
    ...
    <div className="bg-gradient-to-r from-purple-50 to-blue-50 p-8 rounded-2xl shadow-sm ring-1 ring-gray-100">
      <div className="max-w-2xl mx-auto text-center">
        ...
        <p className="text-gray-600 mb-6">
          Estamos trabalhando para tornar o AutomateContábil útil para todos. Assine nossos e-mails para ser notificado sobre
          nossos planos e novas funcionalidades. Sem marketing, anúncios ou spam.
        </p>
        <form onSubmit={handleSubmit} className="flex flex-col gap-4 max-w-md mx-auto">
          <div className="flex flex-wrap items-center justify-center gap-4">
            ...
            >
              {status === "loading" ? "Inscrevendo-se..." : "Assinar"}
            </button>
          </div>
          {message && (
